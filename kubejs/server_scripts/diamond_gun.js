...
ItemEvents.firstLeftClicked(event=>{
	...
	if (item.id=="kubejs:diamond_gun" && item.getCustomData()["custom:gun_jammed"]==true && player.isCrouching()) {
		item.shrink(1)
		event.server.runCommandSilent("execute at "+event.player.uuid+" run playsound kubejs:item.diamond_gun.fart player @a[distance=..20] " + event.player.position().x + " " + event.player.position().y + " " + event.player.position().z + " 1.0")
	...
	if (item.id == "kubejs:diamond_gun") {
		event.server.runCommandSilent("execute at "+event.player.uuid+" run playsound kubejs:item.diamond_gun.click player @a[distance=..5] " + event.player.position().x + " " + event.player.position().y + " " + event.player.position().z + " 1.0")
		item.setLore([
