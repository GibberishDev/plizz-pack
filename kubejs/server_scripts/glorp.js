BlockEvents.rightClicked(event => {
	...
	if (block.id.endsWith("button")) {
		...
		if (checkAltar(block, world, server)) {
			...
			world.runCommandSilent("execute at "+event.player.uuid+" run playsound minecraft:block.beacon.power_select master @a")
			event.server.persistentData.glorp = true
			world.runCommandSilent('execute in '+world.dimension+' run summon item ' + block.pos.x + " " + block.pos.y + " " + block.pos.z + ' {"Item":{"id":"kubejs:glorp"},Glowing:1b}')
			world.runCommandSilent("team add glorp")
			world.runCommandSilent("team modify glorp color green")
...
let glorpPhrases = [
	{"color":"#7bff00","text":"<Glorp> I am Glorp"},
	{"color":"#7bff00","text":"<Glorp> Hi"},
	{"color":"#7bff00","text":"<Glorp> Hello"},
	{"color":"#7bff00","text":"<Glorp> Zeta Reticulians are stupid"},
	{"color":"#7bff00","text":"<Glorp> I got my saucer license at age 4! Yes... I stole it from my dad but thats just details"},
	{"color":"#7bff00","text":"<Glorp> You look weird"},
	{"color":"#7bff00","text":"<Glorp> I want pizza"},
	{"color":"#7bff00","text":"<Glorp> There is a non 0 chance I will explode"},
	{"color":"#7bff00","text":"<Glorp> Thank you for holding me"},
	{"color":"#7bff00","text":"<Glorp> I crashed my starship into a lake"},
	{"color":"#7bff00","text":"* Glorp yawns"},
	{"color":"#7bff00","text":"<Glorp> I think 7tv got access to my onlypaws and leaked all of my photos"},
	{"color":"#7bff00","text":"<Glorp> ⊑⟒⌰⌰⍜"},
	{"color":"#7bff00","text":"<Glorp> ⍙⊑⍜ ⏃⍀⟒ ⊬⍜⎍"},
	[{"color":"#7bff00","text":"<Glorp> "},{"color":"#7bff00","text":"Hello","obfuscated":true}],
	[{"color":"#7bff00","text":"<Glorp> "},{"color":"#7bff00","text":"hi","obfuscated":true}],
	{"color":"#7bff00","text":"<Glorp> My cousin Blorp was not nice to me when we were small"},
	{"color":"#7bff00","text":"<Glorp> I have so much money on my outercosmic™ bank balance. If you get me to their atm I will pay you"},
	[{"color":"#7bff00","text":"<Glorp> "},{"color":"#7bff00","text":"[Click Me]","clickEvent":{"action":"open_url","value":"https://steamcommunity.com/sharedfiles/filedetails/?id=3454978597"}}],
	{"color":"#7bff00","text":"<Glorp> I am fugitive in all 78 outergalactically recognized star regions"},
	{"color":"#7bff00","text":"<Glorp> 42"},
	{"color":"#7bff00","text":"<Glorp> Not gonna sugarcoat this. You humans are kinda ugly"},
	{"color":"#7bff00","text":"<Glorp> I'm bouta glorp"},
	{"color":"#7bff00","text":"<Glorp> Why do cats allow you to have them as pets?"},
	{"color":"#7bff00","text":"<Glorp> Did you know, we are actually in a simulation?"},
	{"color":"#7bff00","text":"<Glorp> BEHIND YOU!"},
	...
	{"color":"#7bff00","text":"<Glorp> The inner machinations of my mind are an enigma"}
	
]
let glorpNoGunPhrases = [
	[
		{"color":"#7bff00","text":"<Glorp> Can I have a "},
		{"color":"#ff4800","text":"gun","bold":true,"underlined":true},
		{"color":"#7bff00","text":"?"}
	],
	{"color":"#7bff00","text":"<Glorp> I assure you, you can trust me with a firearm"},

]
let glorpGunPhrases = [
	{"color":"#7bff00","text":"<Glorp> I like this gun"},
	{"color":"#7bff00","text":"<Glorp> I got 57 more god damn rounds in this 4 round magazine"},
	{"color":"#7bff00","text":"<Glorp> I ate gunpowder"},
	{"color":"#7bff00","text":"* Glorp cocks the gun","italic":true},
	{"color":"#7bff00","text":"<Glorp> Aye! i'm walkin' here!","italic":true},
	{"color":"#7bff00","text":"<Glorp> BAM! and you're dead..."},
]


ItemEvents.firstRightClicked(event=>{
	if (event.item.id != "kubejs:glorp" && event.item.id != "kubejs:glorp_with_gun" || !canTalk) return
	glorpTalk(event.server, event.player, event.item.id == "kubejs:glorp_with_gun")
	event.server.runCommandSilent("execute in " +event.getLevel().dimension.toString()+ " at "+event.player.uuid+" run playsound kubejs:item.glorp.voice player @a[distance=..5] " + event.player.position().x + " " + event.player.position().y + " " + event.player.position().z + " 1.0")
...
ItemEvents.dropped(event=>{
	...
	ent.setGlowing(true)
	event.world.runCommandSilent("team add glorp")
	event.world.runCommandSilent("team modify glorp color green")
	event.world.runCommandSilent("team join glorp "+ent.uuid)
	if (event.item.id == "kubejs:glorp") {
		...
		setTimeout(()=>{
			event.world.runCommandSilent("damage " +event.player.uuid+ " 0.01")
			event.world.runCommandSilent("execute at "+event.player.uuid+" run playsound kubejs:item.glorp.gunshot player @a[distance=..20] " + event.player.position().x + " " + event.player.position().y + " " + event.player.position().z + " 0.5")
			event.player.tell([
...
var canTalk = true
function glorpTalk(server, player, hasgun) {
	...
	server.runCommandSilent("tellraw "+player.username+" "+JSON.stringify(phraseArr[(Math.floor(Math.random()*phraseArr.length))]))
	canTalk = false
